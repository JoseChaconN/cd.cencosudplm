...
class ProductosController extends Controller
{
    ...
    public function search(Request $request)
    {
        //
        $productos_q = [];
        if(!empty($request->input('nombre_producto')) || !empty($request->input('codigo_producto'))){
            $nombre_producto=$request->input('nombre_producto');
            $codigo_producto=$request->input('codigo_producto');
            $productos_q=Producto::where('nombre', 'LIKE' , "%$nombre_producto%")->where('sap', 'LIKE' , "%$codigo_producto%")->get();
        }
        return response()->json(['isSuccessful' => TRUE, 'productos' => $productos_q]);
    }
}
